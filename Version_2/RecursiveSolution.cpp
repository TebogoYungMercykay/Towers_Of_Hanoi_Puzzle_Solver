...
void RecursiveSolution<T>::solveGame(){
    if (RecursiveSolution<T>::validateGame() == true){
...
bool RecursiveSolution<T>::containsLabel(T label){
    bool temp = (containsLabelt1(label, 1) == false && containsLabelt1(label, 2) == false && containsLabelt1(label, 3) == false);
    if (temp == true){
        return false;
    }
    return true;
...
template <typename T>
bool RecursiveSolution<T>::containsLabelt1(T label, int tower){
    if(tower == 1){
